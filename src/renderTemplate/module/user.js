const User = (alias, user, selectedUserId = -1, place = -2, index, emoji) => {
    return `
    <div class="user-template ${alias}-user ${(selectedUserId == user['id'] && place != 0 && place != 1 && place != 2) || (selectedUserId == user['id'] && alias=='vote') ? `user-selected ${alias}-selected` : ''}">
        ...
        <div class="user-name ${alias}-name">${alias != 'chart' ? `${user.name.split(' ')[0]} <br>` : ''} ${alias == 'chart' ? user.name : ''} ${alias != 'chart' ? user.name.split(' ')[1] : ''}</div>
